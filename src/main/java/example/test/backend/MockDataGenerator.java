...
public class MockDataGenerator {
    ...
    static List<Asset> createAssets(List<Category> categories) {
        List<Asset> assets = new ArrayList<Asset>();
        for (int i = 0; i < 100; i++) {
            Asset p = createAsset(categories);
            assets.add(p);
        }
    ...
    private static Asset createAsset(List<Category> categories) {
        Asset p = new Asset();
        p.setId(nextAssetId++);
        
        p.setBarCode(new BigInteger(8,1,random));

        p.setCost(new BigDecimal((random.nextInt(2500) + 50) / 10.0));
